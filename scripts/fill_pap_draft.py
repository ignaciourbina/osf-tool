...
Usage:
    python scripts/fill_pap_draft.py [--output OUTPUT.docx]
"""
...
# Resolve paths relative to the osf-tool root (one level up from scripts/)
TOOL_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(TOOL_ROOT))

from osf_workflow import parse_osf_form, write_osf_form_to_docx, VersionManager

OSF_TEMPLATE = TOOL_ROOT.parent / "OSF Preregistration.docx"
DEFAULT_OUTPUT = TOOL_ROOT / "output" / "PAP_Trait_Aggression_DRAFT.docx"
VERSIONS_DIR = TOOL_ROOT / "versions"
...
HYPOTHESES = (
    ...
    "H2 (Direct effect above political identities): The effect of trait aggression on "
    "far-right vote choice persists after controlling for partisan affect, "
    "indicating a direct candidate-appeal pathway beyond partisan sorting.\n\n"
    ...
    "physical aggression will be the stronger predictor of right vote choice.\n\n"
    "H4 (Partisan mediation): Trait aggression partially operates through "
    "partisan strength. That is, higher trait aggression predicts stronger "
    "partisanship on the side of the right-wing populist, which in turn "
    "predicts vote choice.\n\n"
...
def main():
    ...
    record = vm.save(form, "pap-draft-v4", "Revised hypotheses: H2 reframed above political identities, H3 physical aggression as primary predictor, H4 partisan strength rewrite, dropped cross-country mediation prediction")
